...
LastEditTime: 2022-09-24 19:45:51
'''
import os
import subprocess
...
def task_launcher(args):
    ...
    mixup, loss, loss_norm, aux_loss, aux_loss_w, aux_loss_norm, lr, enc, n_data, id = args
    data_list = [f"slot_rHz_{i}" for i in range(n_data)]

    with open(os.path.join(root, f'slot_rHz{n_data}_mixup-{mixup}_loss-{loss}_enc-{enc}_tv-{aux_loss_w:.4f}_id-{id}.log'), 'w') as wfid:
        exp = [
            ...
            f"--plot.interval=10",
            f"--plot.dir_name={model}_{exp_name}_rHz{n_data}_mixup-{mixup}_{loss}_{enc}_tv-{aux_loss_w:.4f}_{id}",
...
if __name__ == '__main__':
    ...
    tasks = [[1, "cmae", True, "tv_loss", 0.005, True, 0.001, "exp", 10, 1]] #
